...
bajo la licencia Apache, la cual permite utilizar Dr. Scratch de forma simultánea
...
En el momento que \texttt{urls.py} nos dirige a una función -recordar que en Python no
...
apoyar al fichero \texttt{views.py} con \textit{otra serie de ficheros en los que se implementaban
funciones auxiliares}. Así se permite observar fácilmente el flujo de la plataforma
y \textbf{aumentar su modularidad}.
Los otros ficheros que apoyan a \texttt{views.py} se pueden ver en la 
...
detallan únicamente los modelos generados por nosotros y 
...
llamado \textit{Nathan}\footnote{\url{https://github.com/nathan/getsb2}}, un servidor 
...
Con el objetivo de describir claramente cuál ha sido mi aportación al 
proyecto, quiero recordar que \textit{Dr. Scratch ha sido desarrollado 
por varias personas}. 
...
llamada \texttt{analyze\_project}, cuya tarea es abrir un pipe por cada plugin de
...
Si la implementaición no ha sido programada
...
Con el cambio de apariencia de la página principal, había conseguido
 entender como funcionaba  \textit{Bootstrap}. Y me encontraba
...
compañera Eva se incorporará. Cuando ella comenzó,
...
Por ello, se implementó una página que fuera mostrada al usuario en esta situación, la cual puede verse en la \textit{Figura~\ref{figure:error}}.
...
Este último paso es el que se vió modificado, mostrando ahora 3 
...
modales, que aparecen al clicar en las palabras resaltadas en rojo.
...
Dichas ventanas modales, se muestran al clicar en las palabras \textbf{url} y \textbf{proyecto}, teniendo al final dos ventanas diferentes:
...
Presenté
la plataforma en la conferencia e hice énfasis en la fucnionalidad. Esto nos permitió
...
un botón rojo, en la parte superior derecha, que indica ``ORGANIZACIONES''. Al clicar en dicho enlace, nos redirige al recurso \texttt{/sign\_up\_organization}
...
registradas y cononocer los errores que se han producido para 
solventarlos.
...
Al elegir un idioma y clicar en el botón \textbf{``Cambiar idioma''} se pide al servidor
...
el fichero \textit{django.po} de su respectivo idoma, que fue 
...
Cuando un usuario clica en el botón \textit{Regístrate} se 
...
alguno de los campos no coincide la fución \texttt{form.is\_valid()} devolverá \texttt{FALSE}.
...
\begin{enumerate}
    \item La función \texttt{show\_dashboard()} es la función a la que se llama
            cuando se elige un proyecto y se clica en el botón de analizar.            Su lógica se presenta en la \textit{Figura~\ref{figure:show-dashboard}} y se encarga de mostrar al usuario la información referente al análisis:
             \begin{itemize}
...
mecanismo cambia. Al clicar en el botón de análisis se llamará a la función 
...
objetivos alcanzados y cuáles no han podido realizarse por algún motivo
...
Al ser un proyecto tan grande y con tantas personas involucradas, hay que tener
en cuenta el esfuerzo de coordinación que esto supone, además de otras tareas
realizadas sumadas al desarrollo de la plataforma.

En primer lugar, el \textbf{objetivo general} que era tener una plataforma web en 
producción 24x7 que no presentara problemas y realizara la funcionalidad básica
ha sido alcanzada con creces. En los 6 meses de julio a diciembre de 2015, la
...
herramienta muy útil para numerosos docentes y alumnos. En el caso de los 
docentes hemos recibido inmensidad de correos dándonos las gracias
