...
proyectos realizados en \textit{Scratch} -lenguaje orientado a la enseñanza-
 aportando feedback sobre determinados aspectos relacionados con el 
\textit{Pensamiento Computacional}. El \textbf{objetivo} 
del proyecto es dar soporte tanto a maestros como estudiantes
 en sus primeros pasos en la programación con Scratch.
...
cloud de Microsoft llamada Azure y la licencia Apache. Por debajo de ello, tenemos un servidor 
...
cualificados}~\cite{gareis2014skills}, en los que se demandará -prácticamente en todos- tener nociones de
...
lógica. Estas, no únicamente son útiles para aprender a programar, sino \textit{en muchísimas
de las demás asignaturas del currículo pueden servir para involucrar al alumnado}~\cite{calao2015developing}
...
Medios del MIT}, y dirigido por \textit{Mitchel Resnick}~\cite{resnick2009scratch}. 
Su principal 
...
y es un analizador estático de proyectos programados en Scratch.

Mis tutores partiendo de la base de Hairball, programaron el plug-in  \textbf{Mastery}
que ofrece información de \textbf{aspectos del Pensamiento Computacional}:
...
Ambos han realizado algunos papers~\cite{moreno2014automatic} en los que explican
...
Como toda aplicación \textit{Cliente-Servidor}, Dr. Scratch tiene las 3 capas de desarrollo:
...
Señalar que esta sección no trata de explicar cómo se han implementado las
...
me llevó algún tiempo entenderla, ya que era necesario además incluir el \textbf{módulo
...
bajo la licencia Apache, la cual permite utilizar Dr. Scratch de forma simultanea
por numerosos usuarios teniendo estos la percepción de ser los únicos usándolo.\\


\textbf{Django} \\

La \textit{versión actualmente instalada de Django es la 1.7} ya que la 1.8 está aún en 
desarrollo. 
...
\textit{urls.py} es básicamente una lista en la que mediante expresiones regulares se
indica qué recursos son servidos por nuestra aplicación. 

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/django-funcionamiento}
...
funcionamiento es el mostrado en el diagrama de la \textit{Figura~\ref{figure:urls}}.
     ...
     \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/urls}
...
según el modelo de Django en el fichero \textit{views.py}, se debe hacer distinción según
...
\textit{Figura~\ref{figure:views}} se observa el funcionamiento interno.
     ...
     \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/views}
...
el fichero \textit{models.py} del modelo de Django. La declaración de uno de los objetos 
más utilizados por Dr. Scratch se muestra en la \textit{Figura~\ref{figure:models}} 
donde se pueden observar sus campos. \textit{Las distintas clases declaradas en este 
fichero serán instanciadas en las funciones 
para implementar las diferentes funcionalidades de la plataforma}.

     \begin{figure}[H]
        \centering
        \includegraphics[width=0.8\textwidth]{img/models}
...
llamadas \textbf{templates}. Estas plantillas son \textit{renderizadas} con la información que se 
...
quiero recordar que \textit{Dr. Scratch ha sido desarrollado por 
varias personas}. 
Este Proyecto Fin de Carrera es la continuación de una primera
implementación de Cristian Chusing (versión Alpha) pero que aún no había 
terminado todo su
desarrollo y realizaría algunas otras tareas de forma paralela.
Y además ha sido desarrollado en paralelo con las implementaciones realizadas
...
\textit{Figura~\ref{figure:fase-i}}. Señalar que en esta fase del proyecto, 
no se encontraban realizando el proyecto ninguno de mis dos compañeros.

     \begin{figure}[H]
        \centering
        \includegraphics[width=1.0\textwidth]{img/fase-i}
        \caption{Fase I del proyecto Dr. Scratch}
        \label{figure:fase-i}
    \end{figure}
...
había demasiada cantidad, esta primera tarea fue muy dura. Es complicado 
...
procesar las salidas de Hairball las cuales se pueden observar en la
\textit{Figura~\ref{figure:hairball}}.

     \begin{figure}[H]
        \centering
        \includegraphics[width=1.05\textwidth]{img/hairball}
        \caption{Salidas de los plugins de Hairball}
        \label{figure:hairball}
    \end{figure}

Como se puede observar, las salidas no están normalizadas y tuve que
...
Para implementar el análisis de Hairball se necesitó la programación de una función 
llamada \textit{analyze\_project} cuya tarea es abrir un pipe por cada plugin de
Hairball que se ejecuta y guardar la información del plugin en una variable
llamada \textit{result[Nombre del plugin]}, después esa variable la procesaba en cada 
una de las funciones creadas para cada plugin llamadas \textit{proc\_[Nombre del plugin]}
y va guardando en el diccionario la información que se muestra en el dashboard
otorgada por Hairball.
...
páginas aceptable, procedí a generar un fichero logFile.txt dentro de la carpeta
...
escrito en él la información indicada y cerrado.
...
\textbf{Primera puesta en producción} \\
        ...
        Indico ``primera'' ya que se ha realizado en dos ocasiones de formas 
diferentes. Aquí voy a detallar cómo se realizó en esta etapa del proyecto.
...
\textbf{www.drscratch.org}.

En esta etapa, mientras producción estaba alojada en las máquinas de la universidad
\textit{se produjeron numerosos problemas, por algún corte y sobre todo por falta de
memoria}.\\
\end {itemize}
...
a una empresa externa -\textit{TheLogoCompany}- pero la idea estaba clara: debía ser 
...
ideas y aprender de forma dinámica e interactiva}.
    ...
    En este momento, ya que teníamos un logo empecé a estudiar el front-end que
existía hasta el momento y empecé a tomar contacto con Bootstrap, para incluir
el logo y cambiar los colores de la web acorde al logo. Siendo ahora la página 
principal de la web la mostrada en la \textit{Figura~\ref{figure:main-version-beta1}}.

     \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{img/main-version-beta1}
...
\textit{Figura~\ref{figure:dashboard-version-beta1}}.
     ...
     \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/dashboard-version-beta1}
...
su apariencia la mostrada en la \textit{Figura~\ref{figure:main-version-beta2}}
que es la que a día de hoy se conserva con algunas modificaciones.\\
...
Esta página además de la sección principal donde se podía subir el fichero 
del proyecto de Scratch, quisimos ser más cercanos al usuario y se incluyeron
4 secciones más que mostraban información en inglés sobre los siguientes aspectos:
...
\begin{enumerate}
    ...
    \end{figure}
        ...
        implementadas ya y deben ser actualizadas.
    ...
    \begin{figure}[H]
       	\centering
       	\includegraphics[width=0.8\textwidth]{img/contact-us.png}
...
Como se ha visto, en este momento la página se mostraba por completo en inglés.
...
Este objetivo era bastante urgente, ya que queríamos empezar a utilizar la 
herramienta con docentes españoles y en colegios de Madrid para poder observar
como los alumnos utilizaban la plataforma para poder seguir desarrollándola.
...
realizado en esta etapa o en las que he realizado alguna aportación. 
Si la implementaición no ha sido realizada 
...
En este momento ya había conseguido entender como funcionaba Bootstrap con el
cambio de apariencia de la página principal, pero estaba
aprendiendo JavaScript, ya que había numerosos ficheros utilizados en la plantilla
que estaban escritos en este lenguaje y lo iba a necesitar para seguir añadiendo
funcionalidades a la web. 

En concreto una de las funcionalidades que se querían
añadir y que se necesitaba JavaScript era la implementación de una \textbf{barra de 
...
Esta tarea estuve trabajándola durante algún tiempo antes que mi compañera Eva
se incorporará, cuando esto ocurrió le mostré el código que había realizado
 y conseguimos conjuntamente llegar a la siguiente solución.

Se incluyó en la página \textit{main.html} el código HTML con
la ayuda de Bootstrap para mostrar la barra con un estilo determinado y se le 
asignó un id para poder diferenciar esta sección de código del resto de la página.

Con una función de JavaScript que se encuentra en el fichero \textit{progress-bar.js}
 se programó que al mostrar la página se pudiera
visualizar todo el contenido excepto el de la barra de progreso, que puede verse
en la  \textit{Figura~\ref{figure:barra-progreso}}.
...
que estaría asociado a la función de JavaScript que se encargaría de ocultar
el contenido de la página al hacer click en el botón y mostraría el contenido
...
servidor de Scratch, en la petición se indica el identificador númerico que 
...
modifiqué lo implementado por él para incluirlo al código. 
...
se observa en la \textit{Figura~\ref{figure:analisis1}}.
     ...
     \begin{figure}[H]
        \centering
        \fbox{\includegraphics[width=0.25\textwidth]{img/analisis1}}
...
peticiones simultáneas, tuve que migrar el servidor Node.js a nuestra propia
máquina de producción.


\textbf{Corrección de errores en el cuadro} \\

El cuadro anterior permitía dar a cualquiera de los botones anteriores sin haber
introducido ninguna información, por lo que la página nos mostraba error, ya que
...
de gestionar este tipo de situaciones, mostrando la página el cuadro mostrado
en la \textit{Figura~\ref{figure:analisis2}} cuando se pulsa alguno de los
botones de análisis sin ningún proyecto concretado.

     \begin{figure}[H]
        \centering
        \fbox{\includegraphics[width=0.25\textwidth]{img/analisis2}}
        \caption{Gestión de errores en el cuadro de análisis}
        \label{figure:analisis2}
    \end{figure}

En el servidor esto se controla en la función 
\textit{show\_dashboard}, en ella se añade un flag según 
el error detectado y se renderiza con la plantilla \textit{main.html} que se devuelve
...
Para ello necesité realizar cambios en el fichero \textit{settings.py} y aprender 
...
Por ello, se implementó una página que fuera mostrada al usuario en esta situación
que puede verse en la \textit{Figura~\ref{figure:error}}.

     \begin{figure}[H]
        \centering
        \includegraphics[width=0.6\textwidth]{img/error}
...
utilizada para guardar todos los proyectos analizados. Así tenemos a mano, sin
...
verse en la \textit{Figura~\ref{figure:error404}}
     \begin{figure}[H]
        \centering
        \includegraphics[width=0.6\textwidth]{img/error404}
        \caption{Página de error 404}
        \label{figure:error404}
    \end{figure}

Otra página que si es más útil en nuestra plataforma, es la página de error que
...
\textit{Figura~\ref{figure:error500}}.
     ...
     \begin{figure}[H]
        \centering
        \includegraphics[width=0.63\textwidth]{img/error500}
...
ordenador, algunos usuarios continúan utilizando la versión 1.4
...
\textit{check\_version}, es muy simple porque:
...
Y si la extensión correspondía a la versión 1.4 
realizar el cambio
de versión, realizado en la función \textit{change\_version}. Esto se consiguió
 gracias a una función de Kurt sencilla, \textit{convert(``scratch2.0'')}.\\ 


\textbf{Servir páginas diferentes según el nivel computacional} \\

Algunos docentes que estaban utilizando la herramienta en este momento nos 
aconsejaron que tratáramos de simplificar de algún modo el cuadro de resumen 
con las puntuaciones mostradas en el dashboard, consideraban que eran demasiado 
complicados para algunos alumnos.

Así el equipo de Dr.Scratch consideró oportuno probar a mostrar un resumen de 
la información diferente en función del nivel de Pensamiento 
Computacional obtenido. 

Para ello, se modificó la función \textit{show\_dashboard} que hasta ahora
únicamente comprobaba si había algún error en la subida del proyecto de Scratch
o en la url. Si había algún error, activaba el flag concreto del error y lo 
renderizaba con el template \textit{main.html} para enviarlo al usuario. 
Si no había error, mostraba el dashboard con toda la información del diccionario.

Este último paso es el que se vió modificado, mostrando ahora 3 dashboards 
distintos para diferenciar tres niveles de Pensamiento Computacional:
    \begin{itemize}
        \item Si el número de puntos devueltos por el plugin mastery de Hairball
                es mayor o igual a 15, muestra el \textit{dashboard\_master.html}
        \item En el caso en el que los puntos devueltos por el plugin mastery
                es mayor a 7, la función nos muestra el 
                \textit{dashboard\_developing.html}
        \item En otro caso, devuelve el \textit{dashboard\_basic.html}
    \end{itemize}

La idea era mostrar diferentes dashboards, donde si el nivel obtenido era 
básico se mostraría muchísima 
...
la versión Alpha. Pero en preproducción, se diseñaron tres dashboards distintos
        ...
        \item \textbf{Dashboard si el pensamiento computacional es bajo:}
            ...
            \begin{enumerate}
                ...
                \item El cuadro inferior izquierdo indica cuáles son las 
                    ...
                    0. Además añadí un link junto a un icono de una bombilla 
                    en la que al clicar redirigiera a otra página para aprender
                    mediante ejemplos, cómo aumentar esa puntuación.

                \item En el cuadro inferior derecho se muestra en un carrousel 
        ...
        \item \textbf{Dashboard si el pensamiento computacional es medio:}
             ...
             \begin{enumerate}
                ...
                \item En el cuadro inferior derecho muestra ahora, todas las 
                        ...
                        una estrella y la frase de ¡¡¡Bin hecho!!! Y si la puntuación
                        es de 0/3 se añade un link para mejorar como en el dashboard
                        anterior. 
        ...
        \item \textbf{Dashboard si el pensamiento computacional es alto:} 
             ...
             \begin{enumerate}
                ...
                \item   Y al igual que el caso anterior, se pueden ver todas 
                        ...
                        puntuación es de 0 se mantiene el link para aprender
                        a mejorar.
                \item   La diferencia con el caso anterior es que ahora el 
                        carrousel contiene todos los malos hábitos analizados 
                        por Dr. Scratch y
...
llamaba nada la atención el carrousel con los malos hábitos de programación.\\
...
Por ello, actualizamos el cuadro al que se encuentra actualmente en producción.

Se añadió una breve explicación de cómo funciona cada análisis para que se
vea claramente que son dos formas completamente distintas. El resultado
se puede ver en la \textit{Figura~\ref{figure:aclaraciones}}.

                     \begin{figure}[H]
                        \centering
                        \fbox{\includegraphics[width=0.4\textwidth]{img/aclaraciones}}
                        \caption{Aclaración de los métodos de análisis}
                        \label{figure:aclaraciones}
                    \end{figure}

Hay palabras que están resaltadas en rojo debido a
 que eran los conceptos que necesitaban explicación extra. Para ello se 
programaron en la plantilla \textit{main.html} dos ventanas modales que se 
muestran al clicar en las palabras \textbf{url} y \textbf{proyecto} :
 ...
 \begin{itemize}
       \item Una ventana explicativa de qué es una URL cuya apariencia puede 
            verse en la \textit{Figura~\ref{figure:url}}.
                     ...
                     \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.75\textwidth]{img/url}
...
cual nos dió algunos problemas y debían ser solucionados. 
...
Gracias a esta migración Dr. Scratch no ha volvuelto a sufrir problemas por cortes o
...
Al hacerlo facilitan un código de JavaScript que copié en un fichero llamado 
\textit{googleAnalytics.js} y lo incluí en todas las páginas de Dr. Scratch.

Destacar que este fichero no ha sido subido a Github para que no sea utilizado
...
ven en la \textit{Figura~\ref{figure:analiticas}}.
                     ...
                     \begin{figure}[H]
                        ...
                        \caption{Panel de Google Analyticas para Dr. Scratch}
                        \label{figure:analiticas}
                    \end{figure} 

Más adelante se mostrarán las estadísticas con más relevancia en el proyecto.\\
...
El mecanismo es el siguiente, cuando clicas en una de las habilidades, el servidor
trata de ir al recurso \textit{/learn/[habilidad]} y \textit{urls.py} le indica
 que la función que debe ejecutar es \textit{learn(request, page)}. 
...
Al llamar a la función se le pasó como parámetro la página que queremos mostrar,
...
el mecanismo de Django de \textbf{\{\% blocktrans \%\}[contenido]\{\% endblocktrans \%}\},
 en lugar de utilizar el que venía usando desde el principio. \\

\textbf{Botón de ayuda} \\

En uno de los talleres, observé que los niños al llegar a los dashboards no sabían
cómo asimilar la información mostrada. Por ello, se me ocurrió que debíamos hacer
algo para guiarles una vez que llegan a la página, estilo el clip de word. 

Eva implementó con BootstrapTour una guía que les facilitará información para
entender el dashboard. 

Esta funcionalidad aún no ha sido probada para poder 
observar si logra resolver el problema.\\

\textbf{Cuenta de organizaciones} \\

La implementación que quisimos tener realizada antes de llegar a la 
\textit{ScratchConference2015} fue ofrecer la posibilidad de registrar organizaciones.
Esta iniciativa surgió de la petición de varias organizaciones en América que
realizan numerosos campamentos de programación y tienen cientos de proyectos 
que analizar en pocos días.

Conseguimos tener la funcionalidad antes de dicha coferencia en la que presenté
la plataforma y dí enfasis en la fucnionalidad. Esto nos permitió
...
un botón rojo en la parte superior derecha que indica ``ORGANIZACIONES''.

Al clicar en dicho enlace nos redirige al recurso \textit{/sign\_up\_organization}
que nos muestra la página mostrada en la \textit{Figura~\ref{figure:organizaciones}}
y en la cual podemos ingresar si ya nos hemos registrado.

   \begin{figure}[H]
       \centering
       \includegraphics[width=0.75\textwidth]{img/organizaciones}
       \caption{Página principal para organizaciones en Dr. Scratch}
       \label{figure:organizaciones}
   \end{figure} 

Haciendo scroll en la misma página tenemos un formulario para poder registrarnos
y un poco más arriba nos dice las instrucciones que debemos seguir para poder
hacerlo. Todo esto se muestra en la \textit{Figura~\ref{figure:registro-organizaciones}}.

   \begin{figure}[H]
       \centering
       \includegraphics[width=0.75\textwidth]{img/registro-organizaciones}
       \caption{Formulario de registro para organizaciones}
       \label{figure:registro-organizaciones}
   \end{figure}

Cuando nos escribían, introducíamos en la base de datos, con una página muy primitiva, 
los hashkeys que les indicábamos en el correo. Así podían registrarse y dicha
hashkey se eliminaba de la base de datos para que no pudiera ser utilizada por
otro usuario. Así, hemos conseguido tener un control total de las organizaciones
registradas y controlar los errores que se han producido para solventarlos.

Una vez que se han registrado e ingresado en Dr. Scratch para organizaciones
se les muestra la página que se muestra en la 
...
además de los dos análisis por URL y por subida de proyecto hay otro más.
   ...
   \begin{figure}[H]
       \centering
       \includegraphics[width=0.75\textwidth]{img/dashboard-organizaciones}
...
obtenida en otro fichero CSV. Se admiten archivos CSV con 4 formatos diferentes:

\begin{itemize}
    \item Archivo en el que se puede poner en una misma columna el número de 
            identificador de los proyectos de Scratch.
    \item Archivo en el que en una columna se pusiera la URL completa de 
            los proyectos.
    \item Archivo en el que hubiera una primera columna en la que hubiera un
            identificador para el alumno que ha creado el proyecto y otra
            columna para el identificador del proyecto.
    \item Archivo con la misma idea anterior pero con la URL completa del proyecto.  
\end{itemize}

Por si hay alguna duda o confusión de cómo debe ser este archivo programé una
ventana  modal que se muestra en esta página al clicar sobre la palabra 
``fichero CSV'' y que puede verse en la 
...
Señalar que se tuvo en cuenta para la implementación, que podía producirse error
...
dicha situación como una excepción y en el fichero CSV que se devuelve muestra
...
\textit{Figura~\ref{figure:csv}}, con una columna para el identificador
...
Se puede ver cómo en el primer caso, el proyecto ha sufrido algún error durante
el análisis y por ello muestra la frase ``Error analizando el proyecto'', pero
...
Después de la Scratch Conference pudimos obtener muchísimo feedback gracias 
a todas las organizaciones y usuarios que quisieron probar la plataforma. Y 
realizamos nuevas implementaciones debido a varias observaciones que nos 
...
fue que el análisis del CSV tardaba mucho tiempo en realizarse y en algunas
ocasiones terminaba mostrando error, que se debía a falta de memoria.

A Jesús Moreno se le ocurrió que modificásemos el servidor Node.js que nos
...
ya que es lo único que necesitamos para realizar el análsis con Hairball.
...
La velocidad de análisis mediante esta opción aumentó enormemente y desaparecieron
los errores. 

Al modificar el getSb2 se afectó también al análisis mediante URL el cual se 
realiza a mayor velocidad y con menor tráfico. Esto es perfecto para el consumo
del crédito de Azure.\\

\textbf{Traducción de los bloques ``pintados''} \\

Al empezar a traducir la plataforma a otros idiomas me encotré con un problema 
...
Esto tiene una explicación, mi compañera Eva, había tratado de obtener el idoma
para traducir los bloques con JavaScript con la función mostrada en la
\textit{Figura~\ref{figure:javascript-blocks}}.

     \begin{figure}[H]
        \centering
        \includegraphics[width=1.0\textwidth]{img/javascript-blocks}
...
Django y dicha línea obtenia algo coherente. Pero cuando se implementó para 
el catalán, se activaba el mecanismo de traducción pero esa línea devolvía (es).

Tuve que cambiar la función por otra que utilizara AJAX para poder llamar 
en segundo plano a una función que implementé en nuestro servidor que pudiera
decir correctamente el idioma en el que se estaba realizando la petición para
que la traducción funcionase. El cambio puede apreciarse en la
...
En el servidor añadí al fichero urls.py una línea para que se admitiera el
recurso /blocks el cual esta asociado a la función blocks que cambia
el contenido de la variable headers['Accept-Language'] por lo que está almacenado
en request.LANGUAGE\_CODE que es el idioma en el que está configurado el navegador
...
de organizaciones. Por lo que ahora mismo en nuestro servidor tenemos ``organizations''
y ``coders''. Se eligió la palabra coder en lugar de user, porque en Django ya 
...
El objeto user de Django tiene campos y funciones implementadas que hemos utilizado
tales como:
...
para controlar constantemente si el usuario que estaba registrado era una organización
o un programador. Esto lo he hecho mediante una función que se llama
\textit{segmentation} y que es llamada en numerosas partes del código para decidir qué
página es mostrada y el contenido de la misma.

Lo próximo será tener students y teachers, que se extenderán de coder con los
campos añadidos que sean necesarios, quedando el árbol de objetos como muestra
la \textit{Figura~\ref{figure:usuarios}}.

     \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{img/usuarios}
        \caption{Árbol de usuarios de Dr. Scratch}
        \label{figure:usuarios}
    \end{figure}

De esta forma, podemos tener en la base de datos los distintos tipos de usuarios
...
que al clicar en ella muestra un desplegable con los idomas que están disponibles
...
Al elegir un idioma y clicar en el botón ``Cambiar idioma'' se pide al servidor
el recurso /i18n/setlang/ y se observa en el fichero urls.py la siguiente línea:
\textit{url(r'\^i18n/', include('django.conf.urls.i18n'))} la cual activa la vista.

Una curiosidad es que aunque es un formulario, la vista set\_language espera 
ser llamada con un método GET y no POST como es común en los formularios.

El mecanismo interno que realiza la vista es el siguiente:
\begin{enumerate}
    \item Fija la prefencia de idioma al que se ha seleccionado en el parámetro
            \textit{language}.
...
Para poder facilitar esta tarea, tanto a niños pequeños como a personas con
 discapacidad intrlectual, pensamos en realizar un plugin que permitiera realizar
el análisis desde la propia página de Scratch.

El plugin que he realizado es una extensión para el navegador Chrome, que una
vez instalado, como puede verse en la 
\textit{Figura~\ref{figure:plugin-bombilla}}, se ve en la esquina superior 
izquierda del navegador con el símbolo característico de Dr. Scratch:
 la bombilla.
...
Al pulsar en él icono nos muestra el desplegable que vemos en la 
\textit{Figura~\ref{figure:plugin-desplegable}} en el cual ya se ha insertado
la URL actual sin la parte final ``\#editor'' lo que nos permite pulsar 
directamente en el botón ANALYZE! que abrirá otra pestaña y nos mostrará el 
resultado del análisis realizado por Dr. Scratch para este proyecto.
...
Destacar que se ha dejado la opción de poder modificar el contenido de la URL
...
Para lograr la traducción de más idiomas intentamos motivar a los usuarios
de Dr. Scratch a ser los propios traductores. A aquellos usuarios que han 
querido participar en la traducción se les envió el fichero \textit{django.po} de su
respectivo idoma, que fue generado ejecutando en la shell:\\
...
\begin{itemize}
    ...
    \item   el: ha sido utilizado para el griego que estará próximamente en
            producción.   
\end{itemize}
...
puede observarse en la \textit{Figura~\ref{figure:idiomas}}.
     ...
     \begin{figure}[H]
        \centering
        \includegraphics[width=0.5\textwidth]{img/idiomas}
...
poder visualizar las estadísticas, la \textit{Figura~\ref{figure:botones}} 
muestra como queda dicha sección tras la modificación.

     \begin{figure}[H]
        \centering
        \includegraphics[width=0.5\textwidth]{img/botones}
        \caption{Acceso a las páginas de estadísticas, colaboradores y al blog}
        \label{figure:botones}
    \end{figure}

Los botones estadísticas y colaboradores redireccionan a las páginas /statistics
y /collaborators respectivamente. Y el fichero urls.py indica que se encuentran
enlazadas con las funciones statistics y collaborators que muestran las páginas
statics.html y collaborators, de nuevo, respectivamente.

Pero la redirección al blog se realiza mediante un mecanismo especial de Django,
para ello se añadió al fichero urls.py la siguiente línea:\\

    url(r'\^blog\$', 
        RedirectView.as\_view (url='https://drscratchblog.wordpress.com'))\\

\textbf{Modificación de procesador del plugin Dead Code} \\

\textbf{Formulario de registro para usuarios y organizaciones} \\

La forma de registro de las organizaciones, fue muy controlada mediante un 
hashkey que debían solicitarnos por correo electrónico y que nosotros introducíamos
en la base de datos con la ayuda de una página muy primitiva.

Pero esta forma de registro no es la ideal y ya solventados los problemas de 
registro, implementé un formulario de registro del mismo estilo que el que
se realiza en Scratch, ya que hay que tener en cuenta que los usuarios pueden
ser menores de edad.
