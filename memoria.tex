...
El \textbf{objetivo} del proyecto es dar soporte tanto a maestros como estudiantes,
 en sus primeros pasos en la programación con Scratch. Motivar la difusión y 
comprensión de la utilidad de la
programación mientrás se muestra cuáles son 
los errores cometidos desde el punto de vista del pensamiento computacional y 
ayudar así a desarrollar habilidades relacionadas con este campo, como pueden
ser la abstracción o lógica.
...
\begin{itemize}
    \item Conseguir un número de proyectos analizados mayor que 5.000 proyectos.
    \item Alcanzar un número de alumnos registrados en la plataforma superior a 1.500 en total.
    \item Lograr que supere los 120 el número de docentes registrados.
    \item La cifra a superar para visitantes a la web será de 25.000 usuarios únicos.
    \item El número de descargas de la aplicación móvil debe superar las 150.
    \item Reducir las horas del servidor caído a menos de 100.
    \item Traducir la plataforma a al menos 5 lenguas.
\end{itemize}
...
\begin{itemize}
    \item Mostrar una barra de progreso del análisis del proyecto.
    \item Analizar proyectos de las dos versiones de Scratch (1.4 y 2.0).
    \item Realizar el análisis mediante URL del proyecto de la versión online de Scratch.
    \item Introducir cuentas de usuarios.
    \item Implementar el servidor para mostrar tres páginas diferentes, 
    conforme el nivel del programador: bajo, medio o alto.
    \item Diseñar tres páginas distintas para cada nivel computacional.
    \item Migrar a la nube.
    \item Utilizar algún mecanismo de análisis web para tener estadísticas 
            de los accesos a la plataforma.
    \item Programar páginas con información de cómo poder pasar de un nivel a otro utilizando
    distintos bloques de Scratch que demuestran un nivel de pensamiento computacional
    mayor.
    \item Programar una funcionalidad de análisis para organizaciones, permitiéndoles analizar varios proyectos
    de Scratch simultáneamente a través de un fichero .csv y le devuelva toda
    la información del análisis.
    \item Introducir cuentas de organizaciones.
    \item Mostrar páginas de estadísticas que permitan obtener información acerca de
    la evolución de los usuarios y poder realizar un seguimiento.
    \item Programar una extensión para los navegadores que permita analizar un proyecto
    desde la propia página web de Scratch. De forma que únicamente pulsando en 
    un botón, la extensión obtenga la URL del proyecto subido y compartido en
    Scratch mostrado en la página actual y abra una nueva ventana con su 
    análisis en Dr. Scratch.
    \item Incluir un botón que permita al usuario modificar
    el idioma de forma dinámica.
    \item Añadir una página de agradecimiento a los colaboradores: personas que han
    traducido los textos de la web a distintos idiomas, colegios que nos han
    permitido realizar talleres en sus aulas...
    \item Diseñar un dashboard para docentes, que muestre información al maestro sobre
    sus alumnos.
...
y además, al más estilo del lenguaje Scratch, tener una red social, que permita
...
\begin{itemize}
    ...
    Tras la Scratch Conference en Ámsterdam a la que acudió todo el equipo de 
        ...
        \begin{itemize}
            \item Descargar únicamen el .JSON en el análisis por URL para que 
                    se realice más rápidamente.
            \item Traducir los bloques pintados en las páginas para aprender 
                    cómo subir de nivel de pensamiento computacional.
            \item Registrar usuarios.
            \item Incluir un botón para traducir la página según la preferencia del usuario 
                    y no del navegador.
            \item Programar una extensión que permita analizar proyectos sin necesidad de 
                    abrir 
            ...
            \item Traducir los textos que aparecen en la plataforma a más 
                    idiomas: catalán, gallego y portugués.
            \item Diseñar un formulario de registro para usuarios y organizaciones.
        \end{itemize}
...
\texttt{auth\_user}, de la cual heredan algunas de nuestras clases.\\
     ...
     \begin{figure}[H]
        \centering
        \includegraphics[width=1.0\textwidth]{img/base-datos}
...
será explicado más adelante en el capítulo 4, donde se explica el diseño e implementación, 
para comentar qué problemas nos encontramos con la programación inicial y qué cambios
realizamos para solventarlos.\\
...
Hairball es un analizador estático de proyectos programados en Scratch. Lo que 
utiliza para analizar el proyecto, es el fichero .JSON incluido en todo archivo
.SB2 -que es la extensión que tienen todos los proyectos realizados en la interfaz
de Scratch- en el que también se incluyen los sonidos e imágenes que conforma el 
proyecto. 

El citado .JSON, contiene el código del usuario y hairball
puede extraer a través de diferentes plugins información tal como los bloques 
que han sido utilizados y si hay algunos que nunca se llegan a ejecutar. Se 
pueden añadir más plugins para poder analizar otros hábitos de programación de los
usuarios de forma modular. 
...
El \textbf{prototipo inicial} de Dr. Scratch debía:

\begin{itemize}
 \item Diseñar una página principal en la que poder indicar qué fichero quería subirse.
 \item Programar en el servidor el análisis del proyecto de Scratch mediante Hairball.
...
había demasiada cantidad -contaba con 403 líneas de código cuando actualmente tiene más de 2000-  esta primera tarea fue muy dura. Es complicado 
...
Cristian se encargó de poner el código abierto de Nathan (getSb2) en Heroku\footnote{\url{https://www.heroku.com/}},
 lo cual nos dio numerosos problemas más tarde y tuvimos que migrarlo. 
...
Y también realizó la implementación inicial del código, el cual me encargué de 
integrar con el resto del código.
...
ver una página HTML sin estilo que indica \textit{``500 Server error''}. Por
ello, se diseñó la siguiente página personalizada de error 500 que permite
...
offline de Scratch}. Los datos JSON de estos proyectos son diferentes al 
...
de versión, realizado en la función \texttt{change\_version()}. Esto se consiguió
 gracias a una función de Kurt: \texttt{convert()}.\\ 
...
códigos de colores para motivar a los alumnos.
...
plantillas de forma que cada una mostrara únicamente la información que
...
alumnos. En el taller realizado con alumnos pudimos observar que no les 
...
se viera claramente que son dos formas completamente distintas. 
...
que nos dio algunos problemas y debían ser solucionados. \\
...
y de Plain Concepts, empezamos a tratar de buscar la mejor solución.
...
dinámicas que no podían ser importadas del mismo modo y no tenía los conocimientos
necesarios para solucionar el problema. Me puse en contacto con los desarrolladores de
Microsoft que me indicaron que el problema es que era necesario incluir la versión de Windows
de las librerias utilizadas ya que los websites de Azure, ahora llamados Web Apps
corren en máquinas Windows.

Pero en ese momento necesitábamos una solución segura para realizar un test de
usabilidad y decidimos crear una
...
analítica web fue usar \textit{Google Analytics}. Para ponerlo en marcha únicamente 
...
\texttt{googleAnalytics.js} y lo incluí en todas las páginas de Dr. Scratch. 
...
\textit{2015 Scratch Conference} fue ofrecer la posibilidad de registrar organizaciones.
...
Cabe señalar que se tuvo en cuenta para la implementación, que \textit{podía producirse error
...
Después de la Scratch Conference obtuvimos muchísima realimentación gracias 
...
directamente en el botón \texttt{ANALYZE!} que abrirá otra pestaña y nos mostrará el 
...
Se añadieron a la página principal, en la sección de contacto, tres botones para
...
Como ya he comentado, en la Fase I se reprogramaron todos los procesadores
de los plugins de Hairball. Cabe recordar que entonces la salida
...
casos, pero no es así. Cuando en un mismo objeto hay varios 
...
La apariencia del formulario se puede ver en la \textit{Figura~\ref{figure:registro-coder}}. 
     ...
     \begin{figure}[H]
        \centering
        \includegraphics[width=0.8\textwidth]{img/registro-coder}
...
Además, se ha de destacar que Dr. Scratch no ha sido utilizado sólo en España, sino que
...
Cabe destacar de nuevo la importancia de los talleres y jornadas que hemos ido
realizando a lo largo del proyecto, lo que nos ha ido haciendo pivotar según
...
Se ha de señalar la importancia de colaboración para la \textbf{traducción} de la plataforma, lo
...
herramienta muy útil para numerosos docentes y alumnos. Se han  
\textbf{analizado más de 80.000 proyectos}, como puede verse en  
\textit{Figura~\ref{figure:proyectos}}. En el caso de los 
docentes hemos recibido una gran cantidad de correos dándonos las gracias por el 
desarrollo de la herramienta, y algunos han querido participar en la traducción.
...
Me gustaría añadir a todo lo comentado en este documento, que el desarrollo de Dr. Scratch me ha
...
gratificación de ver utilizarlo a personas, especialmente a niños, para
...
cuál es el flujo entre las distintas funciones implementadas, se
muestran tres diagramas de flujo para explicar los distintos tipos de análisis.
